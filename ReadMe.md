...
專為兒童設計的生成式 AI 功能，具備多重安全防護與自我修復機制：

#### 🛡️ 安全防護 (Safety Guardrails)
1.  **Pollinations API 安全模式**：強制啟用 `&safe=true` 參數，過濾 NSFW 內容。
2.  **強制安全後綴 (Safety Suffix)**：所有生成指令強制附加 `, cute kid-friendly style, G-rated, no scary elements, no text`，確保風格統一且溫和。
3.  **負面提示詞防護**：自動注入 `no violence, no nudity` 等負面關鍵字，雙重保險。
4.  **正向引導**：強制使用 **Picar/Disney 3D 風格**，避免生成過於寫實或驚悚的圖片。

#### 🚨 社群檢舉機制 (Community Report)
*   **3票下架**：當圖片被累積檢舉 3 次，系統自動將其下架並封存。
*   **自動重繪**：下架後的單字會被視為「未生成」，下次查詢時自動觸發全新生成流程。

#### 🔧 系統穩定性 (System Stability)
*   **自我修復 (Self-Healing)**：若生成過程中斷導致資料卡在「已策劃 (Planned)」狀態，系統會自動偵測並視為無效資料，觸發重新生成，防止使用者卡在空白頁面。
*   **壞資料清理**：後台具備 `GhostCleanupService`，定期掃描並移除超時未完成的幽靈資料。
*   **手機版面優化**：單頁式(One-screen)緊湊設計，將圖片與輸入框整合於同一畫面。
